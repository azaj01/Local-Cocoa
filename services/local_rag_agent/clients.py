...
class EmbeddingClient:
    ...
    async def encode(self, texts: Sequence[str]) -> list[list[float]]:
        if not texts:
            return []
        base = settings.endpoints.embedding.rstrip("/")
        endpoints: list[str] = []
...
class RerankClient:
    ...
    async def rerank(self, query: str, documents: Sequence[str], top_k: int = 5) -> list[tuple[int, float]]:
        ...
        base = settings.endpoints.rerank.rstrip("/")
        candidates: list[str]
...
class LlmClient:
    ...
    async def complete(
        ...
        *,
        ...
        payload: dict[str, Any] = {"prompt": processed_prompt, "max_tokens": min(max_tokens, settings.llm_context_tokens)}
        if processed_system:
        ...
        base = settings.endpoints.llm.rstrip("/")
        endpoints: list[str] = []
    ...
    async def chat(self, messages: list[dict[str, Any]], *, stream: bool = False) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            response = await client.post(f"{settings.endpoints.llm}/chat", json=payload)
            response.raise_for_status()
    ...
    async def describe_frames(
        ...
        *,
        ...
        if settings.endpoints.vision is None:
    ...
    def _estimate_max_frames_for_context(self) -> int:
        ...
        logger.info(f"🧮 Video context budget: {context_size} tokens, {max_pixels} pixels/frame, ~{tokens_per_frame} tokens/frame → max {max_frames} frames")
        return max_frames
    ...
    async def describe_video_segment(
        ...
        *,
        ...
        logger.info(f"🎬 Video segment {time_range}: using {len(selected_frames)}/{len(frames)} frames (context allows {max_frames})")
...
class TranscriptionClient:
    ...
    async def transcribe(self, audio_bytes: bytes, *, language: str | None = None) -> str:
        if settings.endpoints.transcription is None:
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            response = await client.post(
                f"{settings.endpoints.transcription}/transcribe",
                files=form,
            )
            response.raise_for_status()
            data = response.json()
