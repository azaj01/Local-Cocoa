...
from ..models import FileListResponse, FileRecord, ChunkSnapshot
...
def get_chunk_highlight_png(chunk_id: str, zoom: float = Query(default=2.0, ge=1.0, le=4.0)) -> Response:
    ...
    if (record.extension or "").lower() != "pdf":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Highlight preview is only supported for PDFs.")
...
def delete_file(file_id: str) -> Response:
    ...
    # Delete vectors
    try:
        indexer.vector_store.delete_by_filter(file_id=file_id)
    except Exception:
