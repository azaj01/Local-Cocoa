...
from ..context import get_search_engine, get_storage
from ..models import QaRequest, QaResponse, SearchResponse
from ..search_engine import EmbeddingUnavailableError
...
async def search(
    ...
    try:
        return await engine.search(query.strip(), limit=limit, enable_multi_path=multi_path)
    except EmbeddingUnavailableError as exc:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Embedding service unavailable. Restart the local embedding server and try again.") from exc
