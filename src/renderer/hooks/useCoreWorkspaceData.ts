 ...
 * Provides general workspace data (folders, files, health, etc.)
 * Plugin-specific data should be managed by individual plugin hooks
 */
...
/**
 * Core workspace data hook
 * Manages general workspace state (folders, files, health, indexing progress)
 * Does NOT include plugin-specific data (email, notes, etc.)
 */
export function useCoreWorkspaceData() {
    ...
    const refreshData = useCallback(async () => {
        ...
        if (!api) {
            console.warn('[CoreWorkspaceData] No window.api available!');
            return;
        }
        ...
        try {
            ...
            // Optimize: Skip expensive inventory query during active indexing
            ...
            const [summaryData, folderData, inventoryData, specsData, stageProgressData] = await Promise.all([
                ...
                api.listFolders(),
                shouldSkipInventory
                    ? Promise.resolve({ files, progress, indexing: [] })
            ...
            return { folders: folderData };
        } catch (error) {
            console.error('[CoreWorkspaceData] Error in refreshData:', error);
            if (backendStarting && startupRetryCountRef.current < maxStartupRetries) {
                startupRetryCountRef.current += 1;
                setHealth({
                    status: 'degraded',
    ...
    }, [backendStarting, files, progress]);
